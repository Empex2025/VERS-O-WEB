        ...
        <AppShell rightRail={null}>
            <div className="max-w-4xl mx-auto">
                ...
                <div className="flex flex-col gap-6 pt-2">
                    <Secao titulo="Solicitações Recebidas" itens={SOLICITACOES} />
...
function Secao({ titulo, itens }: { titulo: string; itens: Vinculo[] }) {
    return (
        <section>
            <h2 className="text-sm font-bold text-gray-900 mb-3">{titulo}</h2>
            <div className="flex flex-col gap-3">
                {itens.map((v) => (
                    <button key={v.nome} className="w-full flex items-center gap-3 bg-white rounded-xl border border-gray-100 px-4 py-3 hover:border-[#407BFF]/40 transition-colors text-left">
